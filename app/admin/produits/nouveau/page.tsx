...
export default function NewProduct() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages: ProductImage[] = Array.from(files).map((file) => ({
      ...
      isCardImage: images.length === 0, // La première image est celle de la carte par défaut
    }));
  ...
  const removeImage = (id: string) => {
    setImages((prev) => {
      ...
      // Si on supprime l'image de la carte, définir la première comme carte
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Calculer le pourcentage de réduction si promo
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="brasero-atelier-lbf-60"
              />
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="La pièce maîtresse pour les grandes tablées en extérieur."
              />
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          ...
          <p className="text-sm text-slate-600 mb-4">
            Ajoutez plusieurs images. Glissez-déposez pour changer l'ordre. L'image avec l'étoile ⭐ sera affichée sur la carte produit.
          </p>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {formData.compatibleAccessories.map((slug) => {
                  ...
                  return (
                    <div key={slug} className="flex items-center gap-2 bg-green-50 border border-green-300 rounded-lg px-3 py-2">
                      <img src={imageUrl} alt={displayName} className="w-8 h-8 object-contain rounded" />
                      <span className="text-xs text-green-600 uppercase font-semibold">{displayCategory}</span>
                      <span className="text-sm font-medium text-green-800">{displayName}</span>
                      <button
                        type="button"
                        onClick={() => handleAccessoryChange(slug)}
